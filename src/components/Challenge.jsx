...
const solutions = [
  { text: "AI Resume Analysis", icon: <FileSearch size={16} /> },
  { text: "Career Matching", icon: <Target size={16} /> },
  { text: "Skill Gap Insights", icon: <Activity size={16} /> },
  { text: "Targeted Resources", icon: <Briefcase size={16} /> }
];
    ...
    <section id="challenge" className="relative h-screen min-h-[650px] flex flex-col justify-center bg-white overflow-hidden">
      <div className="max-w-6xl mx-auto px-6 w-full">
        <div className="grid lg:grid-cols-[1.1fr_1fr] gap-10 lg:gap-20 items-center">
          ...
          >
            <span className="text-[10px] md:text-xs font-bold uppercase tracking-[0.2em] text-slate-400 flex items-center gap-3 mb-4">
              <span className="h-px w-6 bg-slate-300 inline-block" /> The Challenge
            </span>

            <h2 className="font-sans text-3xl md:text-4xl lg:text-[2.8rem] font-black leading-[1.1] mb-5 text-[#111827] tracking-tight">
          ...
          <div className="order-1 lg:order-2 flex flex-col gap-4 relative">
            ...
            {problems.map((item, idx) => (
              ...
              >
                {/* 修改1：悬停时的左侧长出条换成了明亮的 oasis-accent 绿色 */}
                ...
                {/* 修改2：图标颜色换成了 oasis-accent，悬停时变成更亮的 oasis-light 薄荷绿 */}
                ...
                <div>
                  <h4 className="font-bold text-base text-white tracking-wide mb-0.5">
                    {item.title}
                  </h4>
